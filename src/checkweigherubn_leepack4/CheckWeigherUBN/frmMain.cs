...
namespace CheckWeigherUBN
{
    ...
    private void LoadAllAlarmFromDatabase()
    {
      AlarmContentDB sqlalarmContentDB = new AlarmContentDB();
      object objRet = sqlalarmContentDB.LoadAll();

      _configuration.list_AlarmContent = (List<AlarmContent>)(objRet);
    }

    private void btnAlarm_Click(object sender, EventArgs e)
    {
      FrmAlarm frmAlarm = new FrmAlarm(_configuration);
      frmAlarm.OnSendSaveDataAlarm += FrmAlarm_OnSendSaveDataAlarm;
      frmAlarm.ShowDialog();
    }

    private void FrmAlarm_OnSendSaveDataAlarm()
    {
      LoadAllAlarmFromDatabase();
    }
